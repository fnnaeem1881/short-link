...
//be followed.
    ...
    define('visualcaptcha/core', [], function() {
        ...
        _imageUrl = function(config, indices) {
            var url = '',
                params = [];

            if (config.numberOfImages < indices.length) {
                config.numberOfImages = indices.length;
            }

            // Is the image index valid?
            for(var i = 0; i < indices.length; i++) {
                if(indices[i] < 0 || indices[i] >= config.numberOfImages) {
                    return url;
                }
            }
            ...
            url = config.url + config.routes.image + '/?images=' + JSON.stringify(indices);
        ...
        return function(config) {
            ...
            imageUrl = function(indexArray) {
                return _imageUrl.call(this, config, indexArray);
            };
    ...
    define('visualcaptcha/templates', [], function() {
        ...
        _asynCreatedCaptchaHTML = function(captcha, language, path, config) {
            var images = '',
                string,
                params,
                self = this,
            ...
            params = {
                imageUrl: captcha.imageUrl(indices),
                indices: indices
            };

            if((params.imageUrl.indexOf('images') > -1) === false || typeof((params.imageUrl.indexOf('images') > -1)) === 'undefined') {
                $('#captcha').html('<div>Error during initialization</div>');
                return;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    var key, index = 0;
                    var namespaceInputHTML = (self.namespaceInput(captcha));
                    var buttonsHTML = (self.buttons(captcha, language, path));

                    for(key in response) {
                        string =
                            '<div class="img">' +
                            '<a class="imgAnchor" href="#"><img src="' + response[key].src + '" id="visualCaptcha-img-' + index + '" data-index="' + index + '" alt="" title="" /></a>' +
                            '</div>';
                        images += string;
                        index++;
                    }
                    ...
                    var script = "<script type='text/javascript'>$(document).ready(function() {$('#captcha a').on('click', function(event) {event.preventDefault();});});</script>";

                    params = {
                        imageFieldName: captcha.imageFieldName(),
                        explanation: (typeof response !== 'undefined' && response.length > 0) ? language.explanation.replace(/ANSWER/, captcha.imageName()) : 'Something went wrong, please try again.',
                        images: images
                    };

                    $('#captcha').html(_t(namespaceInputHTML + string + buttonsHTML + script, params));
                }
            });
        ...
        return {
            ...
            asynCreatedCaptcha: _asynCreatedCaptchaHTML
        };
    });
    /*global define */
    ...
    define('visualcaptcha.jquery', [
        ...
        _loaded = function(config, element, captcha) {
            ...
            templates.asynCreatedCaptcha(captcha, config.language, config.imgPath, config);
