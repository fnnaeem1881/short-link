...
$(document).ready(function ()
{
    ...
    $("input[name='adpack_variant']").change(function() {
        calc_adpacks();
    });
    ...
    $("input[name='adpack_variant']:radio").change(function(e) {
        calc_adpacks();
    });
...
{
    if ($("input[name='adpack_variant']:checked").length == 0) return;

    var currentTimestamp = Math.round($.now() / 1000);
    var count = Number($("#adpacks_number").val());
    var variant = $("input[name='adpack_variant']:checked").val().split('_');
    var variantCost = Number(variant[0]);
    ...
    if (Object.keys(discountGeneral).length > 0 && discountGeneral.percent > 0 && currentTimestamp >= discountGeneral.start && currentTimestamp <= discountGeneral.end) {
        variantCost = Number(variantCost * ((100 - discountGeneral.percent) / 100.0));
    }

    $('#adpack_type_1 span').text(variantCost.toMoney());
    $('#visits').text(variant[1]+' days with '+ count);
    $('#visits').text(count+' ads during '+ variant[1] + ' ' + ((1 == variant[1]) ? 'day' : 'days') + ' ');

    var ads = roundToTwo(count * variant[0]);
    ...
    if (Object.keys(discountGeneral).length > 0 && discountGeneral.percent > 0 && currentTimestamp >= discountGeneral.start && currentTimestamp <= discountGeneral.end) {
        ads = Number(ads * ((100 - discountGeneral.percent) / 100.0));
    } else {
        discounts.forEach(function(discount) {
            if(discount['from'] <= count && discount['to'] >= count) {
                ads-=ads*(discount['discount']/100);
                $('#discount').text('(with '+discount['discount']+'% discount )');
                return;
            }
        });
    }
