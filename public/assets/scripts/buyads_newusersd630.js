...
function calc_adpacks()
{
    ...
    ads = roundToTwo(count * cost);

    if ($('#forced_view').prop('checked')) {
        forcedViewCost = Number(visits * count * parseFloat(settings['forced']));
    }

    calc_fees(ads + count * timer_cost + forcedViewCost);
}
